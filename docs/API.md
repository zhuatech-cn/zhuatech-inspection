...
| POST | `/domain/decision` | 执行企业移动巡检与隐患整改系统专属领域规则 |
...
| `plannedPoints` | int | 计划点位数 |
| `completedPoints` | int | 已巡检点位数 |
| `hazardsFound` | int | 发现隐患数 |
| `hazardsClosed` | int | 已关闭隐患数 |
| `overdueHazards` | int | 逾期隐患数 |
| `locationVerified` | boolean | 定位或扫码已验证 |
| `evidenceComplete` | boolean | 现场证据完整 |
