...
- 独立巡检任务、点位结果和隐患整改数据模型；
- 点位编码唯一，重复提交按幂等方式返回已有结果，防止重复计数；
- 每个点位保存现场读数、证据 SHA-256、巡检人和时间；
- 重大隐患必须先完成停机、警戒等临时控制，才能提交整改；
- 整改人与复核人职责分离，驳回后重新进入待处置状态；
- 未完成全部点位或存在未复核隐患时，服务端禁止关闭任务。
